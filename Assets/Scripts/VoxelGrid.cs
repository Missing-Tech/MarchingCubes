...
public class VoxelGrid : MonoBehaviour
{
    ...
    private void SetCrossings(VoxelStencil stencil, int xStart, int xEnd, int yStart, int yEnd)
    {
        ...
        if (xEnd == resolution - 1) {
            xEnd -= 1;
            crossHorizontalGap = xNeighbour != null;
        }
        if (yStart > 0) {
            yStart -= 1;
        }
        if (yEnd == resolution - 1) {
            ...
            crossVerticalGap = yNeighbour != null;
        }
        Voxel a, b;
        for (int y = yStart; y <= yEnd; y++) {
            ...
            {
                dummyX.BecomeXDummyOf(xNeighbour.voxels[y*resolution],gridSize);
        ...
        if (lastVerticalRow) {
            ...
            for (int x = xStart; x <= xEnd; x++, i++) {
                ...
                stencil.SetHorizontalCrossing(a, b);
                if (crossVerticalGap) {
                    dummyY.BecomeYDummyOf(yNeighbour.voxels[x], gridSize);
                    stencil.SetVerticalCrossing(a, dummyY);
                }
            }
            if (crossVerticalGap) {
                dummyY.BecomeYDummyOf(yNeighbour.voxels[xEnd + 1], gridSize);
                stencil.SetVerticalCrossing(b, dummyY);
            }
            if (crossHorizontalGap) {
                dummyX.BecomeXDummyOf(xNeighbour.voxels[voxels.Length - resolution], gridSize);
                stencil.SetHorizontalCrossing(b, dummyX);
            }
        }
    }
